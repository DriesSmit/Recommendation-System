...
def train(tableData,valueList,algs=['SVD'],iter=200):
    function_mappings = {
    ...
    for i, curAlg in enumerate(algs):
        ...
        if curAlg=='SVDFull':
            ...
            print "Training ", curAlg,"..."
            models[1] = function_mappings[curAlg](tableData,valueList,K=25,steps=iter)
        elif curAlg=='SVDFullInc':
            print "Training ", curAlg,"..."
            models[2] = function_mappings[curAlg](tableData,valueList,K=25,steps=iter,alpha=0.000001)
...
def trainIncrementalSVD(data,valueList, K=40, steps=10, alpha=0.0002, beta=0.02, alphaReg=True, Q=None, P=None):
    ...
    for step in xrange(steps):
        ...
        # start = time.time()
        # print("Ready")
        stepVal = step*len(valueList)
        for indexVal,valLoc in enumerate(valueList):
            if stepVal+indexVal >= check:
                # print("Start: ",len(valueMap),". Value map[0]: ",valueMap[0])
                check += steps*len(valueList) * 0.01
                '''mae = 0.0
                rse = 0.0
                ...
                perc = round((indexVal+step*len(valueList)) * 100.0 / (steps*len(valueList)), 1)
...
def trainFullIncSVD(data,valueList, K=40, steps=10, alpha=0.0002, beta=0.02):#Combine FullInc to use both the previous functions. Save code
    ...
    model = trainIncrementalSVD(data,valueList, K=K, steps=steps, alpha=alpha, beta=beta,alphaReg=False, Q=Q, P=P)

    mae = 0.0
    rse = 0.0
    for valLoc in valueList:
    ...
    mae = mae / len(valueList)
    rse = rse / len(valueList)
...
# If only the SVD incremental algorithm is going to be used, one might place it in a separate function.
...
def incSVDrecommend(model,movieIDList,userMap,movieMap,userID,K=100):
    # Get the user's predictions
    ratings = []
    P = model[0]
    Q = model[1]
    ...
    # The list function is quite fast do to overlocating space for future use. If faster speeds are necessary
    # one might need to implement a normal array.
    ratings = []

    for movieID in movieIDList:
        movieLoc = movieMap[movieID]

        ratings.append((np.dot(P[userLoc, :], Q[:, movieLoc]),movieID))
    ratings.sort()
    ratings.reverse()
    ratings = ratings[0:K]
...
def incSVDSave(model):
    np.save("SVDIncP.npy",model[0])
    np.save("SVDIncQ.npy",model[1])

def incSVDLoad():
    P = np.load("SVDIncP.npy")
    Q = np.load("SVDIncQ.npy")
    model = []
    model.append(P)
    model.append(Q)
