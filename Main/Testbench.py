...
# Maybe rename all the movie variables to item to make it sound more generic.
...
def createData(ratingsLoc,userLoc,movieLoc,seperator,seperator2='::'):
    ...
    for i in range(len(values)):
        if i % takeRate > 0:
            valueList.append((userMap[int(values[i][0])],movieMap[int(values[i][1])]))
    ...
    return hashTrainData,tableTrainData,hashTestData,userMap,movieMap,valueList
...
# Database with 20 million ratings
...
# Use this function to import the 20m ratings
#trainHashData, trainTableData,testHashData,userMap,movieMap,valueMap = createData20ml(ratingsFile,movieLocation) #20m ratings


# Database with 1 million ratings
...
# Use this function to import the 100k and 1m ratings
...
# models[1] = rs.incSVDLoad()

# Test all the algorithms listed
result,runTime = evaluate(trainHashData,trainTableData,testHashData,models,userMap,movieMap,testPerAlg=150,algs=algs)


# Example of making recommendations
# Find the top 5 recommendations in the first 500 movies.
# The first value in each row is the rating, while the second value is the movieID.
# Make sure this list only has items the user has not seen yet.

'''itemIDs = np.arange(1,500)      # Get list numbered from 0 to 499
numRec = 5                      # Number of recommendations to make
userID = 700                    # User id for which to make recommendations
ratings = rs.incSVDrecommend(models[1],itemIDs,userMap,movieMap,userID,K=numRec)
print(ratings)'''

# Here we output and plot the results
